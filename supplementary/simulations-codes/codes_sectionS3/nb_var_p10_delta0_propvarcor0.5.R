...
filename_SI_2cl  <- "results/nb_var/p=10/selective_inference_2cl_delta=0_prop_var_cor=0.5"
filename_SI_4cl  <- "results/nb_var/p=10/selective_inference_4cl_delta=0_prop_var_cor=0.5"
filename_multimod <- "results/nb_var/p=10/multimod_delta=0_prop_var_cor=0.5"
filename_merge_4cl <- "results/nb_var/p=10/merge_selective_inference_4cl_delta=0_prop_var_cor=0.5"
filename_ttest <- "results/nb_var/p=10/ttest_4cl_delta=0_prop_var_cor=0.5"
...
for (i in 1:nsimu){
  sigma <- matrix(corr, nrow = round(prop_var_cor*p), ncol = round(prop_var_cor*p))
  diag(sigma) <- 1
  X.cor <- rbind(rmvnorm(n=n/2, mean = rep(0, round(prop_var_cor*p)), sigma = sigma),
                 rmvnorm(n=n/2, mean = rep(delta,round(prop_var_cor*p)), sigma = sigma))
  X.non.corr <- matrix(NA, nrow = n, ncol = p-round(prop_var_cor*p))
  X.non.corr <- matrix(NA, nrow = n, ncol = p-round(prop_var_cor*p))
  for (k in 1:ncol(X.non.corr)){
    X.non.corr[,k] <- c(rnorm(n/2, mean = 0), rnorm(n/2, mean = delta))
  }
  X <- cbind(X.cor, X.non.corr)
